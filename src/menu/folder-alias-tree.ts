...
export class FolderAliasTreeDataProvider
  implements vscode.TreeDataProvider<FANode>, vscode.FileSystemProvider
{
  private _onDidChangeFile: vscode.EventEmitter<
    vscode.FileChangeEvent[] | undefined
  >;
  private _onDidChangeTreeData: vscode.EventEmitter<
    FANode | undefined | null | void
  ...
  getTreeItem(element: FANode): vscode.TreeItem | Thenable<vscode.TreeItem> {
    ...
    const relativelyPath = treeItem.resourceUri?.path.substring(
      this.workspace.uri.path.length + 1
    );
  ...
  async getChildren(element?: FANode): Promise<FANode[]> {
    ...
    if (this.workspace) {
      ...
      return children.map(([name, type]) => ({
        uri: vscode.Uri.file(path.join(this.workspace.uri.fsPath, name)),
        type
      }));
    }
  ...
  get onDidChangeFile(): vscode.Event<vscode.FileChangeEvent[]> {
    return this._onDidChangeFile.event as vscode.Event<
