...
1. 打开 **命令面板** (`Ctrl+Shift+P` / `Cmd+Shift+P`)
2. 输入 **"Search Alias"**（或 "搜索别名"）并选择
3. 浏览或输入关键词筛选别名列表
4. 选择别名即可在编辑器中打开对应文件

> **注意**：搜索仅匹配别名名称，覆盖所有工作区文件夹。如果未配置任何别名，将显示"未找到别名"提示。
...
- **`src/command/delete-alias.command.ts`** - 删除别名的命令处理器
- **`src/command/search-alias.command.ts`** - 按别名搜索并打开文件的命令处理器
